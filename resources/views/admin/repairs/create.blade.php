...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow">
        <div class="p-6 border-b">
            <h1 class="text-xl font-bold text-gray-800">Добавяне на нов ремонт</h1>
        </div>

        <form action="{{ route('admin.repairs.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div>
                ...
                <select id="client_id" name="client_id"
                    ...
                    <option value="{{ $client->id }}"
                        ...
                        {{ old('client_id', $selectedClient->id ?? request('client_id')) == $client->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <select id="car_id" name="car_id"
                    ...
                    @if($selectedClient)
                        @foreach($selectedClient->cars as $car)
                        <option value="{{ $car->id }}" {{ old('car_id', $selectedCar->id ?? request('car_id')) == $car->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <textarea id="repair" name="repair" rows="3"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('repair') border-red-500 @enderror"
                    placeholder="Опишете извършения ремонт...">{{ old('repair') }}</textarea>
                @error('repair')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" id="part" name="part" value="{{ old('part') }}"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" id="work_cost" name="work_cost" value="{{ old('work_cost') }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                ...
                <div>
                    ...
                    <input type="number" step="0.01" id="part_cost" name="part_cost" value="{{ old('part_cost') }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            ...
            <div class="flex justify-end gap-3 pt-4 border-t">
                ...
                <button type="submit" class="px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition">
                    <i class="fa-solid fa-save mr-2"></i>Добави ремонт
                </button>
            </div>
        </form>
    </div>
