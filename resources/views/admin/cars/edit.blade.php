...
@section('title', 'Редактиране на ' . $car->brand . ' ' . $car->model)
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow">
        <div class="p-6 border-b">
            <h1 class="text-xl font-bold text-gray-800">Редактиране на кола</h1>
            <p class="text-gray-600 mt-1">{{ $car->brand }} {{ $car->model }} - {{ $car->plate }}</p>
        </div>

        <form action="{{ route('admin.cars.update', $car->id) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select id="client_id" name="client_id"
                    ...
                    <option value="{{ $client->id }}" {{ old('client_id', $car->client_id) == $client->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="plate" name="plate" value="{{ old('plate', $car->plate) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="vin_num" name="vin_num" value="{{ old('vin_num', $car->vin_num) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="number" id="hp" name="hp" value="{{ old('hp', $car->hp) }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                </div>
                ...
                <div>
                    ...
                    <select id="fuel" name="fuel"
                        ...
                        <option value="Бензин" {{ old('fuel', $car->fuel) == 'Бензин' ? 'selected' : '' }}>Бензин</option>
                        <option value="Дизел" {{ old('fuel', $car->fuel) == 'Дизел' ? 'selected' : '' }}>Дизел</option>
                        <option value="Газ" {{ old('fuel', $car->fuel) == 'Газ' ? 'selected' : '' }}>Газ</option>
                        <option value="Бензин+Газ" {{ old('fuel', $car->fuel) == 'Бензин+Газ' ? 'selected' : '' }}>Бензин+Газ</option>
                        <option value="Електрически" {{ old('fuel', $car->fuel) == 'Електрически' ? 'selected' : '' }}>Електрически</option>
                        <option value="Хибрид" {{ old('fuel', $car->fuel) == 'Хибрид' ? 'selected' : '' }}>Хибрид</option>
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <input type="url" id="image" name="image" value="{{ old('image', $car->image) }}"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            </div>
            ...
            <div>
                ...
                <textarea id="additional_info" name="additional_info" rows="3"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('additional_info', $car->additional_info) }}</textarea>
            </div>
